...
import { ArrowLeft, ShoppingBag, RefreshCw, Building2, Gem } from 'lucide-react';
    ...
    <div className="min-h-screen px-4 py-6" style={{ background: '#05070d' }}>
      <div className="flex items-center gap-3 mb-5">
        ...
        <div>
          ...
          <h1 className="text-lg font-black text-white flex items-center gap-2">
            <ShoppingBag className="w-5 h-5 text-[#8b5cf6]" /> 내 거래 내역
          </h1>
        </div>
      </div>
      ...
      {loading ? (
        ...
        <div className="text-center py-16">
          ...
          <p className="text-slate-500 font-bold mb-1">거래 내역이 없습니다</p>
          <p className="text-[11px] text-slate-600">거래를 시작하면 여기에 내역이 표시됩니다.</p>
              ...
              <div key={order.id} className="rounded-2xl overflow-hidden" style={{ background: '#0d1220', border: `1px solid ${color}15` }}>
                <div className="px-4 py-2.5 border-b flex items-center justify-between" style={{ borderColor: `${color}10`, background: `${color}08` }}>
                  <div className="flex items-center gap-1.5">
                    ...
                    <span className="text-[10px] font-black uppercase tracking-wider" style={{ color }}>{STATUS_KO[order.status] || order.status}</span>
                  </div>
                  <span className="text-[8px] text-slate-500">{order.ordered_at ? new Date(order.ordered_at).toLocaleDateString('ko-KR') : '—'}</span>
                </div>
                <div className="p-4">
                  ...
                  <div className="grid grid-cols-3 gap-2">
                    <div className="bg-[#151c2e] rounded-xl p-2.5 text-center">
                      <p className="text-[7px] text-slate-500">유형</p>
                      <p className="text-[9px] font-black text-white">{TYPE_KO[order.order_type] || order.order_type}</p>
                    </div>
                    <div className="bg-[#151c2e] rounded-xl p-2.5 text-center">
                      <p className="text-[7px] text-slate-500">수량</p>
                      <p className="text-[11px] font-black text-white">{order.quantity}</p>
                    </div>
                    <div className="bg-[#151c2e] rounded-xl p-2.5 text-center">
                      <p className="text-[7px] text-slate-500">총액</p>
