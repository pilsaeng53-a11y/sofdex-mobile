import React, { useState } from 'react';
import { ACCOUNT_TYPES } from '@/data/futuresTradingAssets';
import { Check } from 'lucide-react';
import { useLang } from '@/components/shared/LanguageContext';

export default function FuturesAccountTypes() {
  const [selectedAccount, setSelectedAccount] = useState('pro');
  const { t } = useLang();

  const featureComparison = [
    { key: 'acct_feature_max_lev',           standard: '1:100',          pro: '1:100',           raw: '1:100' },
    { key: 'acct_feature_min_dep',           standard: '$100',           pro: '$1,000',          raw: '$5,000' },
    { key: 'acct_feature_spread',            standard: '100%',           pro: '75%',             raw: '50%' },
    { key: 'acct_feature_fee',               standard: '$1.0/lot',       pro: '$0.75/lot',       raw: '$3.5/lot (ECN)' },
    { key: 'acct_feature_swap',              standard: '0.02%',          pro: '0.015%',          raw: '0.01%' },
    { key: 'acct_feature_lot',               standard: '0.01',           pro: '0.01',            raw: '0.01' },
    { key: 'acct_feature_watch',             standard: '✓',              pro: '✓',               raw: '✓' },
    { key: 'acct_feature_oneclick',          standard: '✓',              pro: '✓',               raw: '✓' },
    { key: 'acct_feature_analytics',         standard: 'Basic',          pro: 'Full',            raw: 'Full' },
    { key: 'acct_feature_api',               standard: '✗',              pro: '✗',               raw: '✓' },
    { key: 'acct_feature_dedicatedsupport',  standard: 'Email',          pro: 'Email/Chat',      raw: '24/7 Phone' },
    { key: 'acct_feature_risk',              standard: 'Standard',       pro: 'Advanced',        raw: 'Professional' },
  ];

  const benefits = [
    'acct_benefit_spreads',
    'acct_benefit_leverage',
    'acct_benefit_tools',
    'acct_benefit_support',
    'acct_benefit_execution',
    'acct_benefit_signals',
  ];

  return (
    <div className="px-4 py-6 max-w-lg mx-auto space-y-6 pb-20">
      {/* Header */}
      <div className="space-y-2">
        <h1 className="text-3xl font-bold">{t('acct_types_title')}</h1>
        <p className="text-sm text-slate-400">{t('acct_types_subtitle')}</p>
      </div>

      {/* Account Cards */}
      <div className="space-y-3">
        {Object.values(ACCOUNT_TYPES).map((account) => (
          <div
            key={account.id}
            onClick={() => setSelectedAccount(account.id)}
            className={`relative rounded-2xl overflow-hidden cursor-pointer transition-all ${
              selectedAccount === account.id
                ? 'ring-2 ring-[#00d4aa] scale-[1.02]'
                : 'hover:ring-2 hover:ring-[#00d4aa]/30'
            }`}
          >
            <div className={`absolute inset-0 bg-gradient-to-br ${account.color} opacity-10`} />
            <div className={`absolute inset-0 rounded-2xl border ${selectedAccount === account.id ? 'border-[#00d4aa]/50' : 'border-[rgba(148,163,184,0.1)]'}`} />
            <div className="relative p-5 space-y-4">
              <div className="flex items-start justify-between">
                <div>
                  <div className="flex items-center gap-2 mb-1">
                    <span className="text-2xl">{account.icon}</span>
                    <h3 className="text-xl font-bold text-white">{account.name}</h3>
                  </div>
                  <p className="text-xs text-slate-400">{account.description}</p>
                </div>
              </div>

              <div className="grid grid-cols-2 gap-3">
                <div className="bg-[rgba(0,212,170,0.1)] border border-[#00d4aa]/20 rounded-lg p-2.5">
                  <p className="text-[9px] text-slate-500 mb-0.5">{t('acct_max_leverage')}</p>
                  <p className="text-sm font-bold text-[#00d4aa]">1:{account.features.max_leverage}</p>
                </div>
                <div className="bg-[rgba(59,130,246,0.1)] border border-[#3b82f6]/20 rounded-lg p-2.5">
                  <p className="text-[9px] text-slate-500 mb-0.5">{t('acct_min_deposit')}</p>
                  <p className="text-sm font-bold text-[#3b82f6]">${account.features.min_deposit}</p>
                </div>
                <div className="bg-[rgba(139,92,246,0.1)] border border-[#8b5cf6]/20 rounded-lg p-2.5">
                  <p className="text-[9px] text-slate-500 mb-0.5">{t('acct_spread_multiplier')}</p>
                  <p className="text-sm font-bold text-[#8b5cf6]">{(account.features.spread_multiplier * 100).toFixed(0)}%</p>
                </div>
                <div className="bg-[rgba(236,72,153,0.1)] border border-[#ec4899]/20 rounded-lg p-2.5">
                  <p className="text-[9px] text-slate-500 mb-0.5">{t('acct_fee_per_lot')}</p>
                  <p className="text-sm font-bold text-[#ec4899]">${account.features.fee_per_lot}</p>
                </div>
              </div>

              <button className={`w-full py-2.5 rounded-lg font-bold text-sm transition-all ${
                selectedAccount === account.id
                  ? 'bg-[#00d4aa]/20 text-[#00d4aa] border border-[#00d4aa]/30'
                  : 'bg-[#151c2e] text-white border border-[rgba(148,163,184,0.1)] hover:border-[#00d4aa]/20'
              }`}>
                {selectedAccount === account.id ? t('acct_selected') : t('acct_select')}
              </button>
            </div>
          </div>
        ))}
      </div>

      {/* Selected Account Benefits */}
      {selectedAccount && (
        <div className="glass-card rounded-2xl p-5 space-y-4">
          <h3 className="text-sm font-bold text-[#00d4aa]">
            {ACCOUNT_TYPES[selectedAccount.toUpperCase()].name} {t('acct_benefits_title')}
          </h3>
          <div className="space-y-2">
            {benefits.map((key, idx) => (
              <div key={idx} className="flex items-center gap-2">
                <Check className="w-4 h-4 text-green-400 flex-shrink-0" />
                <span className="text-xs text-slate-300">{t(key)}</span>
              </div>
            ))}
          </div>
          <button className="w-full bg-[#00d4aa]/20 hover:bg-[#00d4aa]/30 border border-[#00d4aa]/30 text-[#00d4aa] font-bold py-2.5 rounded-lg transition-all text-sm">
            {t('acct_open_account')} — {ACCOUNT_TYPES[selectedAccount.toUpperCase()].name}
          </button>
        </div>
      )}

      {/* Full Comparison Table */}
      <div className="space-y-3">
        <h3 className="text-sm font-bold text-slate-400 px-1">{t('acct_full_comparison')}</h3>
        <div className="glass-card rounded-2xl overflow-hidden">
          <div className="overflow-x-auto">
            <table className="w-full text-xs">
              <thead className="bg-[#151c2e] border-b border-[rgba(148,163,184,0.1)]">
                <tr>
                  <th className="px-3 py-2.5 text-left font-bold text-slate-400">{t('acct_feature_feature')}</th>
                  <th className="px-3 py-2.5 text-center font-bold text-slate-400">{t('futures_standard')}</th>
                  <th className="px-3 py-2.5 text-center font-bold text-[#00d4aa]">{t('futures_pro')}</th>
                  <th className="px-3 py-2.5 text-center font-bold text-slate-400">{t('futures_raw')}</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-[rgba(148,163,184,0.06)]">
                {featureComparison.map((row, idx) => (
                  <tr key={idx} className="hover:bg-[#1a2340]/50 transition-all">
                    <td className="px-3 py-2.5 text-slate-400 font-semibold">{t(row.key)}</td>
                    <td className="px-3 py-2.5 text-center text-slate-300">{row.standard}</td>
                    <td className="px-3 py-2.5 text-center font-bold text-[#00d4aa]">{row.pro}</td>
                    <td className="px-3 py-2.5 text-center text-slate-300">{row.raw}</td>
                  </tr>
                ))}
              </tbody>
            </table>
          </div>
        </div>
      </div>

      {/* Info Boxes */}
      <div className="space-y-2">
        <div className="glass-card rounded-xl p-4 border-l-4 border-[#00d4aa]">
          <p className="text-xs font-bold text-white mb-1">💡 Pro Tip</p>
          <p className="text-[9px] text-slate-400">{t('acct_tip_pro')}</p>
        </div>
        <div className="glass-card rounded-xl p-4 border-l-4 border-[#8b5cf6]">
          <p className="text-xs font-bold text-white mb-1">🚀 Raw ECN</p>
          <p className="text-[9px] text-slate-400">{t('acct_tip_raw')}</p>
        </div>
        <div className="glass-card rounded-xl p-4 border-l-4 border-[#3b82f6]">
          <p className="text-xs font-bold text-white mb-1">📊 {t('acct_faq_change_q')}</p>
          <p className="text-[9px] text-slate-400">{t('acct_tip_upgrade')}</p>
        </div>
      </div>

      {/* FAQ */}
      <div className="space-y-3">
        <h3 className="text-sm font-bold text-slate-400 px-1">{t('acct_faq_title')}</h3>
        <div className="space-y-2">
          {[
            { q: t('acct_faq_spread_q'), a: t('acct_faq_spread_a') },
            { q: t('acct_faq_change_q'), a: t('acct_faq_change_a') },
            { q: t('acct_faq_swap_q'),   a: t('acct_faq_swap_a') },
          ].map((item, idx) => (
            <details key={idx} className="glass-card rounded-lg p-3 cursor-pointer group">
              <summary className="font-semibold text-xs text-white flex items-center justify-between">
                {item.q}
                <span className="text-slate-500 group-open:rotate-180 transition-transform">▼</span>
              </summary>
              <p className="text-[9px] text-slate-400 mt-2">{item.a}</p>
            </details>
          ))}
        </div>
      </div>
    </div>
  );
}