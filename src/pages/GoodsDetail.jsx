...
import { PRODUCTS, USDT_RATE, toUSDT, addToCart } from '../data/goodsProducts';
...
export default function GoodsDetail() {
  ...
  const usdtAmount = toUSDT(total + delivery);
    ...
    <div className="min-h-screen pb-8" style={{ background: '#05070d' }}>
      ...
      <div className="px-4 space-y-4">
        ...
        <div>
          ...
          <p className="text-[10px] text-slate-500">(≈ {toUSDT(product.price)} USDT)</p>
        </div>
        ...
        <div className="rounded-2xl p-4" style={{ background: 'rgba(0,212,170,0.05)', border: '1px solid rgba(0,212,170,0.1)' }}>
          ...
          <div className="space-y-1.5">
            <div className="flex justify-between"><span className="text-[10px] text-slate-500">상품 가격</span><span className="text-[10px] text-white font-semibold">{(product.price * qty).toLocaleString()}원</span></div>
            ...
            <div className="flex justify-between pt-1 border-t border-[rgba(59,130,246,0.1)]">
              <span className="text-[9px] text-slate-500">테더 환산 금액</span>
              <span className="text-[11px] font-black text-[#3b82f6]">{usdtAmount} USDT</span>
            </div>
