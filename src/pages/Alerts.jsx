...
const ALERT_TYPES = [
  { id: 'price_up',    label: 'Price Target (Up)',      icon: TrendingUp,  color: 'text-emerald-400', bg: 'bg-emerald-400/10' },
  { id: 'price_down',  label: 'Price Target (Down)',    icon: TrendingDown,color: 'text-red-400',     bg: 'bg-red-400/10' },
  { id: 'pct_move',    label: '% Move Alert',           icon: Activity,    color: 'text-[#00d4aa]',   bg: 'bg-[#00d4aa]/10' },
  { id: 'news',        label: 'Breaking News',          icon: Newspaper,   color: 'text-amber-400',   bg: 'bg-amber-400/10' },
  { id: 'governance',  label: 'Governance Proposal',    icon: Vote,        color: 'text-violet-400',  bg: 'bg-violet-400/10' },
  { id: 'rwa',         label: 'RWA Asset Alert',        icon: Building2,   color: 'text-purple-400',  bg: 'bg-purple-400/10' },
  { id: 'liquidation', label: 'Liquidation Spike',      icon: Zap,         color: 'text-orange-400',  bg: 'bg-orange-400/10' },
  { id: 'whale',       label: 'Whale Activity',         icon: Activity,    color: 'text-blue-400',    bg: 'bg-blue-400/10' },
];
  ...
  return (
    <div className="min-h-screen pb-6">
      <div className="px-4 pt-4 pb-3">
        ...
        <p className="text-[11px] text-slate-500">Get notified instantly on price moves, whale activity & more</p>
      </div>
      ...
      {showCreate && (
        <div className="px-4 mb-4">
          <div className="glass-card rounded-2xl p-4 border border-[#00d4aa]/10 space-y-3">
            <p className="text-xs font-bold text-white">Create New Alert</p>
            <div>
              <p className="text-[10px] text-slate-500 mb-1.5">Alert Type</p>
            ...
            <div className="grid grid-cols-2 gap-2">
              <div>
                <p className="text-[10px] text-slate-500 mb-1.5">Asset</p>
              ...
              <div>
                <p className="text-[10px] text-slate-500 mb-1.5">Value / Threshold</p>
                <input
      ...
      <div className="flex gap-1.5 px-4 mb-4">
        {['active', 'notifications'].map(t => (
          <button key={t} onClick={() => setTab(t)}
            ...
            {t === 'active' ? `Active Alerts (${alerts.filter(a => a.active).length})` : 'Notifications'}
          </button>
        ))}
      </div>
      ...
      {tab === 'active' && (
        <div className="px-4 mt-5">
          <p className="text-[10px] font-bold text-slate-500 uppercase tracking-wider mb-2.5">Available Alert Types</p>
          <div className="grid grid-cols-2 gap-2">
            {ALERT_TYPES.map(t => {
              const Icon = t.icon;
              return (
                <div key={t.id} className={`glass-card rounded-xl p-3 flex items-center gap-2 border border-[rgba(148,163,184,0.04)]`}>
