 ...
 *
 * COMPLETELY SEPARATE from FuturesReferral / PartnerHub / CommissionDist.
 * This system is exclusively for approved SOF token sales partners.
...
import { UserPlus, List, BarChart3, Users } from 'lucide-react';

const TABS = [
  { id: 'register', label: 'Register Customer', icon: UserPlus },
  { id: 'manage',   label: 'Customer Management', icon: Users },
...
export default function SOFSalesPartnerDashboard() {
  ...
  async function checkPartnerAccess() {
    ...
    try {
      const partners = await base44.entities.ApprovedSalesPartner.filter({
  ...
  const loadSubmissions = useCallback(async () => {
    ...
    setLoadingData(true);
    try {
      const data = await base44.entities.SOFSaleSubmission.filter(
    ...
    <div className="px-4 py-6 max-w-lg mx-auto space-y-5 pb-24">
      ...
      <div>
        <div className="flex items-center gap-2 mb-1">
          ...
          <span className="text-[9px] font-bold text-[#00d4aa] uppercase tracking-widest">Approved Partner</span>
        </div>
        <h1 className="text-2xl font-bold text-white">SOF Sales Partner</h1>
      ...
      <div className="grid grid-cols-2 gap-3">
        <StatCard label="Total Submissions" value={submissions.length} sub="All time" color="#ffffff" />
        <StatCard label="Total USDT" value={`$${formatNumber(totalUSDT, 0)}`} sub="Customer purchases" color="#00d4aa" />
        <StatCard label="Total SOF Issued" value={`${formatNumber(totalSOF, 2)}`} sub="Calculated quantity" color="#8b5cf6" />
        <div className="glass-card rounded-xl p-4">
        ...
        {TABS.map(tab => {
          ...
          return (
            ...
            >
              ...
              <span className="text-[8px] font-bold leading-tight text-center">{tab.label}</span>
            </button>
          );
        })}
      </div>
          ...
          {loadingData ? (
            ...
            <div className="py-16 flex flex-col items-center gap-4 text-center">
              ...
              <div>
                ...
                <p className="text-xs text-slate-500 mt-1">Register your first customer to begin.</p>
              </div>
              <button
                onClick={() => setActiveTab('register')}
                className="btn-solana px-6 py-2.5 text-xs font-bold rounded-xl"
              >
