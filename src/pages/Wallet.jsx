          ...
          {loading ? (
            ...
            <div className="col-span-2 glass-card rounded-2xl p-4 bg-amber-400/5 border border-amber-400/20">
              <div className="flex items-start gap-2">
                ...
                <div>
                  <p className="text-[10px] font-semibold text-amber-300 mb-1">Unable to Load Balances</p>
                  <p className="text-[10px] text-amber-200/70">{error}</p>
                </div>
              </div>
            </div>
          ) : balances ? (
            <>
              {[
                { label: 'SOL', bal: balances.SOL?.balance || 0, color: 'text-[#00d4aa]' },
                { label: 'USDC', bal: balances.USDC?.balance || 0, color: 'text-blue-400' },
                { label: 'USDT', bal: balances.USDT?.balance || 0, color: 'text-purple-400' },
                { label: 'Total (USD)', val: (balances.SOL?.value || 0) + (balances.USDC?.value || 0) + (balances.USDT?.value || 0), color: 'text-[#00d4aa]' },
              ].map((tile, i) => (
