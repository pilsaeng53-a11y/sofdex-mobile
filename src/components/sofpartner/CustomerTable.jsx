...
function StatusBadge({ status, t }) {
  ...
  const labels = {
    Processing: t('sof_status_processing'),
    Approved:   t('sof_status_approved'),
    Rejected:   t('sof_status_rejected'),
  };
  return (
    <span className={`text-[8px] font-bold px-2 py-0.5 rounded-full ${styles[status] || styles.Processing}`}>
...
export default function CustomerTable({ records, periodFilter, onPeriodFilterChange }) {
  ...
  const PERIOD_OPTIONS = [
    { id: 'all',     label: t('sof_filter_all') },
    { id: 'daily',   label: t('sof_filter_daily') },
    { id: 'monthly', label: t('sof_filter_monthly') },
    { id: 'yearly',  label: t('sof_filter_yearly') },
  ];
    ...
    <div className="space-y-4">
      ...
      <div className="flex flex-col gap-2">
        <div className="relative">
          ...
          <input
            ...
            placeholder={t('sof_table_search_ph')}
        ...
        <div className="flex gap-1 bg-[#0f1525] rounded-xl p-1">
          {PERIOD_OPTIONS.map(p => (
            <button key={p.id}
              onClick={() => onPeriodFilterChange(p.id)}
              className={`flex-1 py-1.5 text-[9px] font-bold rounded-lg transition-all ${
                periodFilter === p.id ? 'bg-[#00d4aa]/15 text-[#00d4aa]' : 'text-slate-500 hover:text-slate-300'
              }`}
            >{p.label}</button>
          ))}
        </div>
      ...
      {filtered.length > 0 && (
        <div className="grid grid-cols-3 gap-2">
          <div className="glass-card rounded-xl p-3 text-center">
            <p className="text-[8px] text-slate-500 uppercase">{t('sof_table_records')}</p>
            <p className="text-base font-bold text-white">{filtered.length}</p>
          </div>
          <div className="glass-card rounded-xl p-3 text-center">
            <p className="text-[8px] text-slate-500 uppercase">{t('sof_table_total_usdt')}</p>
            <p className="text-base font-bold text-[#00d4aa]">${formatNumber(totalAmount, 0)}</p>
          </div>
          <div className="glass-card rounded-xl p-3 text-center">
            <p className="text-[8px] text-slate-500 uppercase">{t('sof_table_total_sof')}</p>
      ...
      {filtered.length === 0 ? (
        <div className="py-10 text-center text-xs text-slate-500">
          {search ? `${t('sof_table_no_results')} "${search}"` : t('sof_table_no_period')}
        </div>
      ) : (
          ...
          {filtered.map(r => (
            <div key={r.id} className="glass-card rounded-xl p-4 space-y-3">
              <div className="flex items-start justify-between gap-2">
                ...
                <StatusBadge status={r.status || 'Processing'} t={t} />
              </div>

              <div className="grid grid-cols-3 gap-2 text-[9px]">
                <div>
                  <p className="text-slate-500">{t('sof_table_purchase')}</p>
                  <p className="font-bold text-white">${formatNumber(r.purchase_amount, 0)} USDT</p>
                </div>
                <div>
                  <p className="text-slate-500">{t('sof_table_sof_price')}</p>
                  <p className="font-bold text-slate-200">${formatNumber(r.sof_unit_price, 4)}</p>
                </div>
                <div>
                  <p className="text-slate-500">{t('sof_table_promo')}</p>
              ...
              <div className="flex items-center justify-between border-t border-[rgba(148,163,184,0.06)] pt-2">
                <div>
                  <p className="text-[8px] text-slate-500">{t('sof_table_sof_qty')}</p>
